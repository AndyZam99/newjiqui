...
const Card: React.FC<CardProps> = ({ title, subtitle, Icon, href }) => {
  return (
    <a
      href={href}
      className="w-full p-4 rounded-[20px] border-[2px] border-pink-300 relative overflow-hidden group bg-white"
    >
      <div className="absolute inset-0 bg-gradient-to-r from-amber-500 to-violet-500 translate-y-[100%] group-hover:translate-y-[0%] transition-transform duration-300" />

      <Icon className="absolute z-10 -top-12 -right-12 text-9xl text-slate-100 group-hover:text-pink-700 group-hover:rotate-12 transition-transform duration-300" />

      <Icon className="mb-2 text-2xl text-pink-700 group-hover:text-white transition-colors relative z-10 duration-300" />
